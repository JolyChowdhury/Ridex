...
$tripadvisor_enabled_option=get_option('tripadvisor_enabled',$option_group);
$yelp_enabled_option=get_option('yelp_enabled',$option_group);
$tiktok_enabled_option=get_option('tiktok_enabled',$option_group);
...
$tripadvisor_enabled = $tripadvisor_enabled_option == 'y';
$yelp_enabled = $yelp_enabled_option == 'y';
$tiktok_enabled = $tiktok_enabled_option == 'y';
...
if (isset($params['show-icons'])) {
    ...
    foreach ($all as $item) {
        ...
        //     $medium_enabled = true;
        // }
        else if (strpos($icon, 'tripadvisor') !== false and $tripadvisor_enabled_option == false) {
            $tripadvisor_enabled = true;
        }else if (strpos($icon, 'yelp') !== false and $yelp_enabled_option == false) {
            $yelp_enabled = true;
        }else if (strpos($icon, 'tiktok') !== false and $tiktok_enabled_option == false) {
            $tiktok_enabled = true;
        }
    }
}
...
$rss_url=get_option('rss_url',$option_group);

if ($rss_url == false) {
    $rss_url = get_option('rss_url', 'website');
}

$tripadvisor_url=get_option('tripadvisor_url',$option_group);

if ($tripadvisor_url == false) {
    $tripadvisor_url = get_option('tripadvisor_url', 'website');
}
$yelp_url=get_option('yelp_url',$option_group);
if ($yelp_url == false) {
    $yelp_url=get_option('yelp_url','website');
}
$tiktok_url=get_option('tiktok_url',$option_group);
if ($tiktok_url == false) {
...
if ($facebook_enabled or $twitter_enabled or $googleplus_enabled or $pinterest_enabled or $youtube_enabled or $linkedin_enabled or $soundcloud_enabled or $tripadvisor_enabled or $yelp_enabled or $tiktok_enabled) {
