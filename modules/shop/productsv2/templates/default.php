...
<?php
    ...
    if(isset($params['data-related_product'])){
        $status = 'related_product';
    }else{
        $status = 'default';
    }
    
    $dataSet = $controller->index($params,$status);
    $hide_paging = $dataSet['hide_paging'];
    $paging_param = $dataSet['paging_param'];
    $per_page_items = $dataSet['per_page_items'];
    $count = $dataSet['count'];
    $categoryUrl = $dataSet['cat_url'];
    $wishlist_id = $dataSet['wishlist_id'];
    $wishlist_slug = $dataSet['wishlist_slug'];
    ...
    <?php if($is_logged and $dataSet['has_wishlist']){
        $pro_ids = json_decode(json_encode($data), true);
        $pro_ids = array_column($pro_ids['data'], 'content_id');
        if(count($pro_ids) > 0){
        ?>
    ...
    <?php if (isset($pages_count) and $pages_count > 1 and $hide_paging): ?>
        <module type="pagination" template="single_button" pages_count="<?php echo $pages_count; ?>" paging_param="<?php echo $paging_param; ?>" product_count="<?=count($data)?>" count="<?=$count?>" current_page="<?=$current_page?>" content_type="product"  cat_url="<?=$categoryUrl?>" wishlist_slug="<?=$wishlist_slug?>" wishlist_id="<?=$wishlist_id?>"/>
    <?php endif;
    ...
    <script type="text/javascript">
        ...
        // Break line if the first word of categroy name is too big
        $('.categorySideBar ul.nav li a').each(function(index, el) {
            let word = el.text.split(" ");
            let firstWordLen = word[0].length;

            if (firstWordLen > 30) {
                $(this).addClass('bigger-category')
            }  
